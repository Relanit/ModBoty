...
class Link(commands.Cog):
    ...
    async def edit(self, ctx):
        ...
        if link in cog.aliases.get(ctx.channel.name, []):
            await ctx.reply(
                f'Название {self.bot._prefix}{link} уже занято категорией {cog.aliases[ctx.channel.name][link]["name"]}'
            )
            return
        ...
        if link in self.links.get(ctx.channel.name, []):
            ...
            for alias in content[1:]:
                ...
                if alias in cog.aliases.get(ctx.channel.name, []):
                    await ctx.reply(
                        f'Название {self.bot._prefix}{alias} уже занято категорией {cog.aliases[ctx.channel.name][alias]["name"]}'
                    )
                    return
