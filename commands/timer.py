...
class Timer(commands.Cog):
    ...
    async def timer(self, ctx):
        ...
        if ctx.command_alias == 'delt':
            values = {'$pull': {'timers': {'link': link}}}
            message = f'Удалён таймер {self.bot._prefix}{link}'
        ...
        for channel in self.timers:
            ...
            for timer in timers:
                if time.time() > self.timers[channel][timer]['cooldown'] and self.messages_from_timer[channel] >= 5:
                    ...
                    if 'announce' in self.timers[channel][timer]:
